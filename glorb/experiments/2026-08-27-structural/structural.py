...
def report(ps, fr, ft, pa=None, pb=None):
    ...
    ha, hb = wledlab.capture_health(fr, SECONDS), wledlab.capture_health(ft, SECONDS)
    for tag, h in (("ref", ha), ("tgt", hb)):
        if h["coverage"] is None or h["coverage"] < 0.9 or h["max_gap"] is None or h["max_gap"] > 2.0:
        ...
        for ps in presets:
            ...
            if offline:
                if not os.path.exists(path) and os.path.exists(path + ".gz"):
                    path += ".gz"
                d = wledlab.load_capture(path); report(ps, d["ref"], d["tgt"]); continue
            fr, ft = wledlab.simultaneous(R, T, SECONDS, preset=ps); time.sleep(3)
